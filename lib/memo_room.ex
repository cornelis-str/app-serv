defmodule Memo_room do
  ...
  def set_quest(room_data, how, quest_id, quest, pid) do
    ...
    |> case do
      nil when how == :add ->
        # Lägg till
        room_data
        |> Map.replace!(:quests, [%{:quest_id => quest_id, :quest => quest} | room_data.quests])
      ...
      index when how == :add ->
        ...
        room_data
        |> Map.replace!(:quests, [
          %{:quest_id => quest_id, :quest => quest} | room_data.quests |> List.delete_at(index)
        ])
  ...
  def set_quest_pics(room_data, how, quest_pic_id, pic, pid) do
    ...
    |> case do
      ...
      index when how == :add ->
        room_data
        |> Map.replace!(:quest_pics, [
          %{:quest_pic_id => quest_pic_id, :quest_pic => pic}
          | room_data.quest_pics |> List.delete_at(index)
        ])
